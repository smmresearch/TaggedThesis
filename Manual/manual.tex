...
Upon testing in Summer 2025, the template compiles on Crixet with tagging, however, it is much slower than installing and running LaTeX on your own computer. Using the draft option (see \autoref{classOptions}) often is highly recommended if you choose this route.
...
Compiling this document will generate the figure as a standalone PDF. This can then be included in your main thesis with \texttt{includegraphics}.
...
Table cells are automatically tagged, but table headers must be tagged.
Since LaTeX cannot tell what are headers without help, you must also declare any header rows or columns using \texttt{tagpdfsetup} inside the table environment.
...
\end{table}
\end{lstlisting}
\subsubsection{Not Using Table Environment}
Most of the time tables will be in a table environment. Occasionally, however, you may not want to use the table environment. In such a case, use \texttt{captionsetup} before the table to ensure proper formatting.

\begin{lstlisting}
% no \begin{table}...\end{table}
\captionsetup{type=table}%if you don't use floating table, you must tell latex that it is a table
\begin{tabular}{...}
    ...
\end{tabular}
\end{lstlisting}
...
It is suggested you use the \texttt{math-packages} document class option (see \autoref{classOptions}).
...
It is suggested you use the \texttt{algorithm-packages} document class option (see \autoref{classOptions}) to have painless tagged algorithms. This option loads the packages algorithm and algpseudocode. While you may try to use other algorithm packages, they may not be compatible with tagging. This option also defines a \texttt{parstate} command which allows for states that must be wrapped onto multiple lines.
...
In the \textbf{rare} case you create a heading \textit{without using the sectioning commands (chapter/section/subsection/...)}, you will manually need to instruct LaTeX to tag it. For example,
\begin{lstlisting}
{
...
Appendices are handled differently in Journal and Traditional. In both cases, appendices are labeled with letters: $A$, $B$, $\ldots$

For a Journal style thesis, simply add another section labeled with ``Appendix $<letter>$'':
\begin{lstlisting}
...
For a Traditional style thesis, the Appendices are additional chapters after the References chapter. In this case, the chapter command will automatically add Appendix $<letter>$ to the title for you.
If you have a single appendix:
\begin{lstlisting}
\singleappendixtitle
\chapter{Appendix Name}
\finishAppendixSetup%must come immediately after chapter 
...%body of the appendix
\end{lstlisting}

If you have multiple appendices, in the \textbf{first} appendix:
\begin{lstlisting}
\appendixtitle
\appendix
\chapter{First Appendix Name}
\finishAppendixSetup%immediately after chapter in the first appendix
...%body of the appendix
\end{lstlisting}

In subsequent appendices, you only need the chapter command:
\begin{lstlisting}
\chapter{Appendix of Second and Onward Appendix}
...%body of the appendix
\end{lstlisting}

In a traditional thesis, all headings \textit{below} chapter (i.e. section, subsection, $\ldots$) in the appendices should be \textit{unnumbered}. To do this, use the starred version of the sectioning commands:
\begin{lstlisting}
...
By default, some long bibliography entries may be broken across two pages. It is ISU's opinion that this is bad. You can prevent this by using the \texttt{bib-no-break} document class option (see \autoref{classOptions}).
...
\begin{itemize}
    ...
    \item bib-no-break: by default, page breaks in the middle of bibliography entries are discouraged, but may happen for a particularly long entry (4+ lines). With this option, a page break will never happen in the middle of an entry, no matter how long. This is "recommended" but not required by Iowa State.
\end{itemize}
...
H floats are a misnomer - they are not actually floating around anymore. H floats are not yet officially supported, but we can fix the tagging ourselves.

For the most useful tagging, you should ask yourself whether you expect the reader to stop reading the text exactly where you placed the float and switch to looking at the float. If so, then you will want to wrap the float in a ``Part'' tag:
\begin{lstlisting}
...
If the float is alone in an appendix, I would suggest tagging it as a float.
...
\begin{lstlisting}
    \newfontfamily\stix{DejaVu Sans}
\end{lstlisting}
to the preamble of thesis.tex where DejaVu Sans is the name of the font. This will \textbf{not} change the font of the whole document. Within the .tex file you can copy and paste: {\stix ☐} and {\stix  ☒} from here to create the unchecked and checked boxes. The curly braces and the \texttt{stix} commands are important.
...
\subsection{Error: "... \@ does not doesn't match definition..."}
First, trying running biber and re-running lualatex a few times. If the error still does not resolve, try deleting all auxillary files and running lualatex $->$ biber $->$ lualatex.
\subsection{Error: Missing  .... \_\_um\_group\_begin}
It appears to happen when you have something like
\begin{lstlisting}
    x_\someCommand{whatever}
\end{lstlisting}
To resolve, add curly braces to the sub/super script:
\begin{lstlisting}
   x_{\someCommand{whatever}} 
\end{lstlisting}
\subsection{Parent-Child Warnings}
In the standards for the PDF, there are rules about which tags in the tree should go under which other tags. For example, a table cell has to be a child of a table row.
Some of these warnings are not a big deal, but there are ways to fix them.
\subsubsection{'P/pdf2' $-->$ 'float/latex'}
This warning can appear with some floats - especially algorithms.
This is easily remedied by adding a blank line before \texttt{begin{algorithm}} or other kind of floating environment if needed.
\subsubsection{Reference $-->$ Formula }
This is caused by math in caption or header (which then ends up in the list of figures/table/contents).
The PDF standards were modified in 2025 to allow this. However, LaTeX has not yet been updated.
I would suggest leaving it as is. However, if it bothers you, it can be worked-around.

If it is caused by math in a caption, wrap the formula in a Span.
For example,
\begin{lstlisting}
    words {\tagstructbegin{tag=Span}$math$\tagstructend}\tagmcend\tagmcbegin{} more words
\end{lstlisting}

If it is caused by math in a header, you should already be using texorpdfstring. Put the Span inside the tex argument:
\begin{lstlisting}
\section{ words \texorpdfstring{ { \tagstructbegin{tag=Span}$math$\tagstructend\tagmcend\tagmcbegin{} } }{<plain text for the pdf metadata} more words }
\end{lstlisting}
Notice, all of
\begin{lstlisting}
\tagstructbegin{tag=Span}$math$\tagstructend\tagmcend\tagmcbegin{}
\end{lstlisting}
is wrapped in a seemingly-extraneous set of {...}. This is necessary.
\subsection{Warning: Patching footnotes failed.}
Do not worry about this warning.
\section{Thesis.tex Breakdown}
\begin{lstlisting}
%Required:DocumentMetadata turns on tagging
\DocumentMetadata{
 lang=en,
 tagging=on,
 pdfversion=2.0,
...
%Optional:Does not put end at the end of algorithm
\PassOptionsToPackage{noend}{algpseudocode}

%12 pt font, loads math and algorithms packages
\documentclass[12pt,math-packages,algorithms-packages]{isuthesistagged}

\usepackage{multirow}%Optional:allows multirow cells in table
\usepackage{pdflscape}%Optional:supports landscape pages

%Optional:Defines Theorem,Definition,Case,etc.
\usepackage{templatedTheoremTypes}

%Sets libertinus as the font (uses unicode)
\usepackage[libertinus]{fontsetup}

%Remaps \bm to to \symbf for bold 
\let\bm\symbf

%Required: biblatex
%Optional:natbib option - allows the use of natbib-style commands
%Optional:style=authoryear - can be set to preferred style 
%In Journal theses, refsection=chapter is required because each
%chapter is separate. In Traditional style, refsection is omitted
%because there is just one bibliography
\usepackage[natbib=true,refsection=chapter,style=authoryear]{biblatex}
\addbibresource{thesisAccessTest.bib}%adds the bib file
\setlength{\bibitemsep}{13.2pt}%changes separation between bib items

%Set the author - to be modified as appropriate
\newcommand{\theAuthor}{Alice Wonder}
%Set the tite 
\twoLineTitle{This is the title of a thesis
submitted to Iowa State University on the first line}{
...
%%The following set the author and title in the thesis
% and in the PDF metadata. No editing required.
\author{\theAuthor}
...
%Optional: Tikz package for figures
\usepackage{tikz}

\begin{document}
\include{Preface/titlepage}%Required:titlepage.tex is included
\frontmattersetup%ensures all preliminary pages are formatted correctly

\include{Preface/dedication}%Optional:Dedication

\tableofcontentsTagged%Required:loads the table of contents
\cleardoublepage \phantomsection
\pagebreak %ensures newpage

\listoftablesTagged %remove if no tables, otherwise required

\cleardoublepage \phantomsection
\listoffiguresTagged %remove if no figures, otherwise required

%Optional nomenclature
%\cleardoublepage \phantomsection
%\nomenclatureTagged

%Required: Adds Chapter in front of chapter on TOC
...
\newpage
\pagenumbering{arabic}
\pagestyle{fancy}%Required:helps set page numbering
%body of the thesis here
